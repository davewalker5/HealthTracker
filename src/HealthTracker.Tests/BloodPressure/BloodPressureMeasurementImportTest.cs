...
namespace HealthTracker.Tests.BloodPressure
{
    ...
    public class BloodPressureMeasurementImportTest
    {
        ...
        {
            var record = $@"""{_person.Id}"",""{_person.Name}"",""{_measurement.Date:dd-MMM-yyyy HH:mm:ss}"",""{0}"",""{_measurement.Diastolic}"",""Optimal""";
            _filePath = DataGenerator.TemporaryCsvFilePath();
            File.WriteAllLines(_filePath, ["", record]);
            await _importer.ImportAsync(_filePath);
        }
        ...
        {
            var record = $@"""{_person.Id}"",""{_person.Name}"",""{_measurement.Date:dd-MMM-yyyy HH:mm:ss}"",""{_measurement.Systolic}"",""{0}"",""Optimal""";
            _filePath = DataGenerator.TemporaryCsvFilePath();
            File.WriteAllLines(_filePath, ["", record]);
            await _importer.ImportAsync(_filePath);
        }
