...
namespace HealthTracker.Tests.AlcoholConsumption
{
    [TestClass]
    public class AlcoholConsumptionMeasurementClientTest
    {
        private readonly string ApiToken = DataGenerator.RandomApiToken();
        private MockHealthTrackerHttpClient _httpClient = new();
        private IAlcoholConsumptionMeasurementClient _client;
        ...
        private readonly HealthTrackerApplicationSettings _settings = new()
        {
            ...
            ApiUrl = "http://server/",
            ApiRoutes = [
                new() { Name = "AlcoholConsumptionMeasurement", Route = "/alcoholconsumptionmeasurement" },
                new() { Name = "ExportAlcoholConsumptionMeasurement", Route = "/export/alcoholconsumption" },
                new() { Name = "ImportAlcoholConsumptionMeasurement", Route = "/import/alcoholconsumption" }
            ]
        };
        ...
        public void Initialise()
        {
            ...
            _client = new AlcoholConsumptionMeasurementClient(_httpClient, _settings, provider.Object);
        }
        ...
        {
            var measurement = DataGenerator.RandomAlcoholConsumptionMeasurement(2025);
        ...
        public async Task ImportTest()
        {
            ...
            var measurement = DataGenerator.RandomAlcoholConsumptionMeasurement(person.Id, beverage.Id, 2025);
            ...
            Assert.AreEqual($"{_settings.ApiUrl}", _httpClient.BaseAddress.ToString());
            Assert.AreEqual(HttpMethod.Post, _httpClient.Requests[0].Method);
            Assert.AreEqual(_settings.ApiRoutes.First(x => x.Name == "ImportAlcoholConsumptionMeasurement").Route, _httpClient.Requests[0].Uri);
        }
        ...
        public async Task ExportTest()
        {
            ...
            Assert.AreEqual($"{_settings.ApiUrl}", _httpClient.BaseAddress.ToString());
            Assert.AreEqual(HttpMethod.Post, _httpClient.Requests[0].Method);
            Assert.AreEqual(_settings.ApiRoutes.First(x => x.Name == "ExportAlcoholConsumptionMeasurement").Route, _httpClient.Requests[0].Uri);
        }
    }
}