...
{
    [TestClass]
    public class AlcoholConsumptionMeasurementExportTest
    {
        ...
        private AlcoholConsumptionMeasurement _measurement;
        private string _filePath;

        [TestInitialize]
        public void Initialize()
        {
            _measurement = DataGenerator.RandomAlcoholConsumptionMeasurement(_person.Id, _beverage.Id, 2025);
        }
        ...
        {
            List<AlcoholConsumptionMeasurement> measurements = [_measurement];
        ...
        {
            var record = $@"""{_person.Id}"",""{_person.Name}"",""{_measurement.Date:dd-MMM-yyyy HH:mm:ss}"",""{_beverage.Id}"",""{_beverage.Name}"",""{(int)_measurement.Measure}"",""{_measurement.Measure.ToName()}"",""{_measurement.Quantity}"",""{_measurement.ABV}"",""{_measurement.Units}""";
            var exportable = ExportableAlcoholConsumptionMeasurement.FromCsv(record);
            Assert.AreEqual(_person.Id, exportable.PersonId);
            Assert.AreEqual($"{_person.Name}", exportable.Name);
            Assert.AreEqual(_measurement.Date, exportable.Date);
            Assert.AreEqual(_beverage.Id, exportable.BeverageId);
        ...
        public async Task ExportAllMeasurementsTest()
        {
            ...
            await context.AlcoholConsumptionMeasurements.AddAsync(_measurement);
            ...
            var exportable = ExportableAlcoholConsumptionMeasurement.FromCsv(records[1]);
            Assert.AreEqual(_person.Id, exportable.PersonId);
            Assert.AreEqual($"{_person.Name}", exportable.Name);
            Assert.AreEqual(_measurement.Date, exportable.Date);
            Assert.AreEqual(_beverage.Id, exportable.BeverageId);
