...
{
    public class BloodOxygenSaturationMeasurementExportService : BackgroundQueueProcessor<BloodOxygenSaturationMeasurementExportWorkItem>
    {
        private const string DateTimeFormat = "yyyy-MM-dd HH:mm:ss";
        private readonly HealthTrackerApplicationSettings _settings;

        public BloodOxygenSaturationMeasurementExportService(
            ILogger<BackgroundQueueProcessor<BloodOxygenSaturationMeasurementExportWorkItem>> logger,
            IBackgroundQueue<BloodOxygenSaturationMeasurementExportWorkItem> queue,
        ...
        /// <returns></returns>
        protected override async Task ProcessWorkItemAsync(BloodOxygenSaturationMeasurementExportWorkItem item, IHealthTrackerFactory factory)
        {
            ...
            MessageLogger.LogInformation("% SPO2 measurement export completed");
        }
        ...
        private void LogExportMessages(BloodOxygenSaturationMeasurementExportWorkItem item, string filePath)
        {
            MessageLogger.LogInformation($"Exporting % SPO2 measurements for the person with ID {item.PersonId} to {filePath}");
