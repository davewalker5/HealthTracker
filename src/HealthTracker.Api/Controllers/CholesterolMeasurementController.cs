...
namespace HealthTracker.Api.Controllers
{
    ...
    public class CholesterolMeasurementController : Controller
    {
        ...
        [HttpGet]
        [Route("{id}")]
        public async Task<ActionResult<CholesterolMeasurement>> Get(int id)
        {
            var measurements = await _factory.CholesterolMeasurements.ListAsync(x => x.Id == id, 1, int.MaxValue);
        ...
        public async Task<ActionResult<IEnumerable<CholesterolMeasurement>>> ListCholesterolMeasurementsForPersonAsync(int personId, string from, string to, int pageNumber, int pageSize)
        {
            ...
            var measurements = await _factory.CholesterolMeasurements.ListAsync(
